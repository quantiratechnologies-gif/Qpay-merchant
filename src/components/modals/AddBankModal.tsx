...
export const AddBankModal: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {/* SAMA & eKYC Verification Status Tag */}
      <div
        style={{
      ...
      >
        ...
        <div style={{ fontSize: '11.5px', color: '#94A3B8' }}>
          {language === 'العربية'
            ? 'يتم التحقق من مطابقة السجل التجاري ورقم الهوية آلياً عبر ساما ونفاذ'
            : 'Account ownership verified with CR & National ID via SAMA Open Banking Directory'}
        </div>
      </div>

      <PrimaryButton onClick={handleAdd} disabled={isLoading}>
        {isLoading
          ? (language === 'العربية' ? 'جاري التحقق عبر ساما والربط...' : 'Authenticating via SAMA eKYC...')
          : (language === 'العربية' ? `توثيق وربط ${displaySelectedBank}` : `Verify & Link ${selectedBank}`)}
      </PrimaryButton>
