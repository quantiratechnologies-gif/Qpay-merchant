...
export const PaymentLinkGeneratorScreen: React.FC = () => {
  ...
  const [generatedLink, setGeneratedLink] = useState('https://riyalpay.sa/pay/lnk_8829104');
  const [copied, setCopied] = useState(false);
  const [isSimulating, setIsSimulating] = useState(false);

  const numAmount = parseFloat(amount) || 0;
  const activeOrderRef = orderRef.trim() || 'ORD-8839';
  const activeCustomer = customerName.trim() || (isAr ? 'عميل مميز' : 'Customer');

  const handleGenerate = (e: React.FormEvent) => {
    e.preventDefault();
    const linkId = 'lnk_' + Math.floor(1000000 + Math.random() * 9000000);
    setGeneratedLink(`https://riyalpay.sa/pay/${linkId}`);
  };
  ...
  const handleShareWhatsApp = () => {
    const text = encodeURIComponent(
      isAr
        ? `مرحباً ${activeCustomer}، إليك رابط الدفع لطلبك ${activeOrderRef} (${merchantInfo.businessName}):\nالمبلغ: ${numAmount > 0 ? numAmount.toFixed(2) : '---'} ر.س\nادفع بأمان عبر أبل باي / البطاقات البنكية / سريع:\n${generatedLink}`
        : `Hello ${activeCustomer}, here is your payment link for ${activeOrderRef} (${merchantInfo.businessName}):\nAmount: SAR ${numAmount > 0 ? numAmount.toFixed(2) : '---'}\nPay securely via Apple Pay / Debit Card / Sarie:\n${generatedLink}`
    );
    window.open(`https://wa.me/?text=${text}`, '_blank');
  };

  const handleSimulateRemotePayment = async () => {
    const finalAmount = numAmount > 0 ? numAmount : 120.0;
    setIsSimulating(true);
    await processMerchantCollection({
      amount: finalAmount,
      paymentMethod: 'payment_link',
      orderRef: activeOrderRef,
      customerMasked: activeCustomer,
    });
    setTimeout(() => {
  ...
  return (
    ...
    >
      ...
      <div style={{ width: '100%', maxWidth: '380px', margin: '14px auto', display: 'flex', flexDirection: 'column', gap: '14px' }}>
        ...
        <PrimaryButton onClick={handleSimulateRemotePayment} disabled={isSimulating}>
          <Sparkles size={16} /> {isAr ? `محاكاة دفع العميل (${formatSaudiCurrency(numAmount > 0 ? numAmount : 120.0, language)})` : `Simulate Customer Paid (${(numAmount > 0 ? numAmount : 120.0).toFixed(2)} SAR)`}
        </PrimaryButton>
      </div>
